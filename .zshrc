...
#~      ↓↓↓ Замените этот блок на собственный ↓↓↓
            ...
            #watch -n 1 tail -n 3 /var/log/wimax.log #смотрим уровень сигнала Yota
        watch -n 1 "dmesg|tail -n 11" #смотрим dmesg

            # ПРОСТО ОЧИСТКА ЭКРАНА
            #clear #недостаток: не видим вывода последней команды, которая отработала перед
                   #срабатыванием заставки

            # ПЕРЕРИСОВЫВАНИЕ ПРИГЛАШЕНИЯ ПО ^C В ТЕРМИНАЛЕ
            #kill -SIGINT $$ & #чревато зависаниями шелла, приглашение просто не рисуется

            # РИСОВАНИЕ АНАЛОГА МОЕГО ПРИГЛАШЕНИЯ
            #        echo -n "
            #
            #
            #\e[1;39m[$USER@$HOST] \e[1;32m$PWD/\e[0m
            #\e[1;39m# \e[0m"

        # РИСУЕМ ПОДОБИЕ ПОСЛЕДНЕЙ СТРОКИ ПРИГЛАШЕНИЯ
        echo -en "\e[1;39m# \e[0m" #этого достаточно, только она проблемная
